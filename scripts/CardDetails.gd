...
func _sync_tasks():
	...
	if has_tasks:
		for task_local in card.tasks:
			var checkitem = CheckitemScene.instance()
			checklist_items_container.add_child(checkitem)
			checkitem.set_model(task_local)
			checkitem.connect("edit_check_item_requested", self, "_on_edit_check_item_requested")
			checkitem.connect("toggled", self, "_on_check_item_toggled", [task_local])
...
func _on_check_item_toggled(_is_toggled, task_local):
	card.update_task(task_local, task_local.title, not task_local.is_done)
